...
export default function InscripcionMovilPage() {
  ...
  async function cargarDatos(id: string) {
    ...
    let programacionData: unknown = null;
    let programacionErrorMensaje = "";

    const programacionRelacion = await supabase
    ...
    if (programacionRelacion.error) {
      programacionErrorMensaje =
        programacionRelacion.error.message ||
        "No se pudo cargar la programación con relaciones.";

      const programacionSimple = await supabase
        .from("programaciones_cursos")
        .select("*")
        .eq("id", id)
        .maybeSingle();

      if (programacionSimple.error) {
        setError(
          `Error cargando el curso: ${
            programacionSimple.error.message || programacionErrorMensaje
          }`
        );
        setProgramacion(null);
      } else if (!programacionSimple.data) {
        setError("No se encontró la programación del curso seleccionada.");
        setProgramacion(null);
      } else {
        setProgramacion(programacionSimple.data as ProgramacionCurso);
      }
    } else if (!programacionRelacion.data) {
      setError("No se encontró la programación del curso seleccionada.");
      setProgramacion(null);
    } else {
      programacionData = programacionRelacion.data;
  ...
  function obtenerDiasComoLista(valor: string | null | undefined) {
    ...
    const mapa = [
      ...
      { clave: "miercoles", tokens: ["miercoles", "miércoles", " mi ", "mi-", "x "] },
  ...
  async function validarInscripcionDuplicada() {
    ...
    if (cedulaLimpia) {
      participantesQuery = participantesQuery.eq("cedula", cedulaLimpia);
    } else if (correoLimpio) {
      participantesQuery = participantesQuery.ilike("correo", correoLimpio);
    } else {
      participantesQuery = participantesQuery.eq("telefono", telefonoLimpio);
    }

    const { data: participantesEncontrados, error: participantesError } =
      await participantesQuery;

    if (participantesError) {
      console.error("Error validando participante:", participantesError);
      setError(`Error validando duplicidad: ${participantesError.message}`);
      return false;
    }

    const participantesIds = (participantesEncontrados || []).map(
      (item) => item.id
    );

    if (participantesIds.length === 0) return true;
    ...
    const { data: inscripcionesExistentes, error: inscripcionesError } =
      await supabase
        ...
        .in("participante_id", participantesIds);
    ...
    for (const inscripcion of inscripcionesExistentes || []) {
      ...
      if (estadoInscripcion.includes("anulada") || estadoInscripcion.includes("cancelada")) {
        continue;
      }
      ...
      if (
        ...
        setError(
          `Este estudiante ya tiene una inscripción registrada para el curso ${
      ...
      if (
        ...
        setError(
          `Este estudiante ya tiene otro curso inscrito en el mismo horario y día (${
